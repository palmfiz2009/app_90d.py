...
if 'init_90d_done' not in st.session_state:
    ...
    defaults = {
        ...
        "wbc_90": None, "hb_90": None, "plt_90": None, "cre_90": None, "egfr_90": None, "crp_90": None,
        "cd_grade_90": "選択してください", "cd_detail_90": "",
        "adj_plan_90": "選択してください", "adj_other_90": "",
        "pfs_intra_uro": "なし", "pfs_intra_uro_detail": [],
        "pfs_recist_status": None, "pfs_recist_date": None, "pfs_recist_site": [],
...
with tab2:
    ...
    with c2:
        adj_opts = ["選択してください", "無治療（経過観察）", "後治療を計画中（未開始）", "EVP継続投与", "ペムブロ維持", "ニボ単剤（術後補助）", "GC/GCarbo", "その他"]
...
with tab3:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.pfs_intra_uro = st.radio("尿路内再発の有無 (膀胱・対側)*", ["なし", "あり"], index=None, horizontal=True)
        if st.session_state.pfs_intra_uro == "あり":
            st.session_state.pfs_intra_uro_detail = st.multiselect("再発部位 (尿路内)", ["膀胱内", "対側腎盂", "対側尿路", "尿道"])
        
        st.divider()
        st.session_state.pfs_recist_status = st.radio("尿路外再発・RECIST進行の有無*", ["なし", "あり"], index=None, horizontal=True)
    ...
    with c2:
        if st.session_state.pfs_recist_status == "あり":
            ...
            st.session_state.pfs_recist_site = st.multiselect("再発・進行部位", ["手術局所", "領域リンパ節", "遠隔リンパ節", "肺", "肝", "骨", "既存転移巣の増大", "その他"])
...
再発評価:
尿路内: {st.session_state.pfs_intra_uro} ({st.session_state.pfs_intra_uro_detail})
